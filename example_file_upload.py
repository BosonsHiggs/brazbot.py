...
class MyBot(DiscordBot):
    ...
    async def on_error(self, data, interaction=None):
...
@bot.command(name="upload", description="Upload a file")
@sync_slash_commands(guild_id=MY_GUILD)
@rate_limit(limit=1, per=60, scope="user")
async def upload_command(ctx, file: Attachment):
    await ctx.defer()
    
    file_data = await file.to_file()
    await ctx.send_followup_message(
